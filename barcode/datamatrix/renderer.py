...
class DataMatrixRenderer:
    ...
    def get_pilimage(self, cellsize, border):
        """Return the matrix as an PIL object"""

        # add the quiet zone (2 x cell width)
        self.add_border(colour=0, width=border)
    ...
    def write_file(self, cellsize, filename, border):
        """Write the matrix out to an image file"""
        img = self.get_pilimage(cellsize, border)
        img.save(filename)

    def get_imagedata(self, cellsize, border):
